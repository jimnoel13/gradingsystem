...
@section('content')
	...
	<div class="btn-group btn-group-justified">
	  <a href="{{ route('faculty.show', $users->id) }}" class="btn btn-default active"><img id="btn-group-img" src="{{ asset('images/ched-logo.png') }}"> <strong>CHEd</strong></a>
	  <a href="{{ route('depedassign.show', $users->id) }}" class="btn btn-default"><img id="btn-group-img" src="{{ asset('images/deped-logo.png') }}"> <strong>DepEd</strong></a>
	...
	<table class="table table-hover">
		...
		<tbody>
			...
			<tr>
				@if($assign->school_dept == 'CHEd')
				<td></td>
				<td>{{ $assign->subject_code }}</td>
				<td>{{ $assign->subject_title }}</td>
				<td>{{ $assign->units }}</td>
				<td>{{ $assign->semester }}</td>
				<td>{{ $assign->year }} - {{ $assign->to }}</td>
				<td>{{ date('M j, Y', strtotime($assign->created_at)) }}</td>
				<td><a href="{{ route('facultysubject.show', $assign->id) }}" class="btn btn-default btn-block"> Manage Student</a></td>
        ...
        <div class="modal-lg">
            <div class="modal-content">
                <div class="modal-body">
					...
					<h4><img src="{{ asset('images/ched-logo.png') }}"> List of CHEd Subjects</h4>
					<br>
						...
						<table class="table table-bordered">
							...
							<tbody>
								...
								<tr>
									<td>{{ Form::text('subject_code', $subject->subject_code, ['class' => 'no-border2', 'readonly']) }}</td>
									<td>{{ Form::text('subject_title', $subject->subject_title, ['class' => 'no-border5', 'readonly']) }}</td>
									<td>{{ Form::text('units', $subject->units, ['class' => 'no-border2', 'readonly']) }}</td>
									<td><a href="#" data-toggle="modal" data-target="#chedassign-{{ $subject->id }}" class="btn btn-success btn-block"> Assign</a></td>
								</tr>

								<!-- Subject Assign Modal -->
								<div class="modal modal-center" id="chedassign-{{ $subject->id }}" tabindex="-1">
							        <div class="modal-dialog">
							            <div class="modal-content">
							                <div class="modal-header">
							                    <h4 class="modal-title"><span class="fa fa-edit"></span> Assign Subject</h4>
							                </div>
							                <div class="modal-body">
											{!! Form::open(['action' => 'FacultySubjectController@store', 'method' => 'POST']) !!}
												...
												<div class="row input-margin-top">
													<div class="col-md-6">
														{{ Form::label('timefrom', 'Start') }}
														{{ Form::text('timefrom', '', ['class' => 'form-control', 'placeholder' => 'eg. 12:00 AM/PM']) }}
													</div>
													<div class="col-md-6">
														{{ Form::label('timeto', 'Ends') }}
														{{ Form::text('timeto', '', ['class' => 'form-control', 'placeholder' => 'eg. 12:00 AM/PM']) }}
													</div>
												</div>
